...
function MovieCard(props){
    ...
    const handleClick = (e) => {
        // Prevent modal from opening when clicking the "Read more" button
        ...
        >
            ...
            {data.Rating !== undefined && (
                <div className="flex justify-end">
                    <div className="flex items-center bg-yellow-100 dark:bg-yellow-900 px-2 py-0.5 rounded-lg shadow-sm text-yellow-700 dark:text-yellow-200 text-xs font-semibold mt-2 mr-2">
            ...
            <div className="p-6">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-2" title={data.Title}>{data.Title}</h2>
                <p className="text-gray-600 dark:text-gray-400 mb-4">Release Date: {data.release_date}</p>
                {data.MovieImage && (
                ...
                {data.Description && (
                    <>
                        <p className="text-gray-700 dark:text-gray-300 text-sm mb-2">{displayText}</p>
                        {isLong && (
                            <button
                                className="text-blue-600 dark:text-blue-400 hover:underline text-sm font-medium focus:outline-none"
                                onClick={(e) => {
                                    e.stopPropagation();
